      ...
      <div className="border-b border-fairway-100 bg-white">
        <div className="container-page grid gap-6 py-10 lg:grid-cols-[1fr_360px] lg:items-end">
          ...
          <div className="grid grid-cols-3 overflow-hidden rounded-lg border border-fairway-100 bg-cream text-center">
            ...
            <MiniStat label="정렬" value={filters.sort === "price" ? "가격순" : filters.sort === "rating" ? "평점순" : "추천순"} />
          </div>
        </div>
      </div>
      ...
      <div className="container-page grid gap-8 py-10 lg:grid-cols-[320px_1fr]">
        ...
        <section className="order-1 lg:order-2">
          <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h2 className="text-xl font-extrabold text-fairway-900">조건에 맞는 프로</h2>
              <p className="mt-1 text-sm font-medium text-fairway-600">
                총 <span className="text-fairway-900">{pros.length}</span>명의 프로가 있습니다.
              </p>
            </div>
          ...
          {pros.length === 0 ? (
            <div className="card p-12 text-center">
              <h3 className="text-lg font-extrabold text-fairway-900">조건에 맞는 프로가 없어요</h3>
              <p className="mt-2 text-fairway-500">지역이나 가격 필터를 조금 넓혀보세요.</p>
            </div>
          ) : (
