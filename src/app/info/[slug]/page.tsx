    ...
    <main>
      <section className="relative overflow-hidden bg-fairway-950 text-white">
        ...
        <div className="container-page relative py-8 sm:py-12">
          <Link
            href="/info"
            className="text-sm font-bold text-fairway-100 hover:text-white"
          >
            골프정보
          </Link>
          <p className="mt-6 text-[13px] font-bold text-gold-200">
            {category.eyebrow}
          </p>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {category.highlights.map((highlight) => (
              <span
                key={highlight}
                className="rounded-full bg-white/12 px-3 py-1 text-xs font-bold text-white backdrop-blur"
              >
                {highlight}
              </span>
            ))}
      ...
      <section className="container-page grid gap-4 py-8 md:grid-cols-3 sm:py-10">
        {category.articles.map((article) => (
          <article
            key={article.title}
            className="rounded-lg border border-fairway-100 bg-white p-4 shadow-sm"
          >
            <span className="rounded-full bg-gold-100 px-2.5 py-1 text-[11px] font-black text-gold-800">
              {article.tag}
            </span>
            <h2 className="mt-3 text-lg font-black text-fairway-900">
              {article.title}
            </h2>
            ...
            <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 className="mt-1 text-xl font-extrabold text-fairway-900">
                  {category.title} 광고 배너
                </h2>
                <p className="mt-1 text-sm text-fairway-600">
                  이 주제를 보는 골퍼에게 어울리는 브랜드와 상품을 사진 배너로
                  연결합니다.
                </p>
              </div>
              <a
        ...
        <div className="rounded-lg border border-fairway-100 bg-white p-4 sm:flex sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-bold text-fairway-900">
              내 상황에 맞는 레슨도 같이 찾아볼까요?
            </p>
            <p className="mt-1 text-sm text-fairway-500">
              정보를 본 뒤 바로 프로 상담이나 맞춤 견적 요청으로 이어갈 수
              있습니다.
            </p>
          </div>
