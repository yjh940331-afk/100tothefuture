...
export function BrandLogo({
  ...
  return (
    <span className={`inline-flex min-w-0 items-center gap-2.5 ${className}`}>
      <span
        className={`relative flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-lg ${styles.mark}`}
      >
        <svg viewBox="0 0 40 40" className="h-9 w-9" fill="none" aria-hidden>
          ...
          />
          <path
            d="M27.7 10.2h7.3l-2.2 3.2 2.2 3.3h-7.3z"
            fill={styles.fairway}
          />
      ...
      {!iconOnly && (
        <span className="min-w-0 leading-none">
          <span
            className={`block truncate text-[15px] font-black ${styles.title}`}
          >
            Future Golf
          </span>
          {!compact && (
            <span
              className={`mt-1 block truncate text-[10px] font-semibold ${styles.subtitle} max-[380px]:hidden`}
            >
              레슨 매칭 플랫폼
            </span>
          )}
