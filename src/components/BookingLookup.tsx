...
export function BookingLookup() {
  ...
  async function lookup(event?: React.FormEvent) {
    ...
    const data = await res
      .json()
      .catch(() => ({ ok: false, error: "응답을 확인할 수 없습니다." }));
    setBusy("idle");
    if (!data.ok) {
      ...
      <form onSubmit={lookup} className="card h-fit space-y-4 p-6">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-fairway-600">
            예약 완료 화면에서 받은 예약번호와 예약 시 입력한 연락처를
            입력하세요.
          </p>
        </div>
        <label>
        ...
        {!booking ? (
          <div className="flex h-full min-h-[260px] flex-col justify-center text-center">
            ...
            <p className="mt-2 text-sm leading-6 text-fairway-500">
              예약번호를 잃어버린 경우 운영자에게 연락처와 예약 프로명을
              알려주세요.
            </p>
          ...
          <div>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <h2 className="mt-1 break-all text-xl font-black text-fairway-900">
                  {booking.id}
                </h2>
              </div>
            ...
            <dl className="mt-6 grid gap-3 sm:grid-cols-2">
              ...
              <Info
                label="접수일"
                value={new Date(booking.created_at).toLocaleString("ko-KR")}
              />
            </dl>
