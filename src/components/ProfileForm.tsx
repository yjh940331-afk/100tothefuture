    ...
    <form onSubmit={submit} className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input className="input" value={form.name} onChange={(e) => set("name", e.target.value)} placeholder="홍길동" required />
        </div>
        <div>
          <label className="label">연락처 *</label>
          <input className="input" value={form.phone} onChange={(e) => set("phone", e.target.value)} placeholder="010-0000-0000" required />
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input className="input" value={form.nickname} onChange={(e) => set("nickname", e.target.value)} placeholder="후기에 표시될 이름" />
        </div>
        <div>
      ...
      <div className="rounded-xl bg-cream p-4">
        ...
        <div className="mt-3">
          ...
          <input className="input" value={form.goal} onChange={(e) => set("goal", e.target.value)} placeholder="예: 드라이버 슬라이스 교정" />
        </div>
      </div>

      <label className="flex items-start gap-2 text-[13px] text-fairway-700">
        <input type="checkbox" className="mt-0.5" checked={form.marketing_agreed} onChange={(e) => set("marketing_agreed", e.target.checked)} />
